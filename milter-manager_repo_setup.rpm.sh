...
finalize_yum_repo ()
{
  ...
  yum install -y pygpgme --disablerepo='milter-manager_repos'
  ...
  if [ "$?" != "0" ]; then
    ...
    sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/milter-manager_repos.repo
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='milter-manager_repos'
  ...
  echo "Generating yum cache for milter-manager_repos..."
  yum -q makecache -y --disablerepo='*' --enablerepo='milter-manager_repos'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh milter-manager_repos
  zypper --gpg-auto-import-keys refresh milter-manager_repos-source
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/milter-manager/repos/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/milter-manager_repos.repo
  else
    yum_repo_path=/etc/yum.repos.d/milter-manager_repos.repo
  fi
